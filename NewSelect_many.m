function PopNew = NewSelect_many(PopDec,PopObj,PopCon,ObjMSE,ConMSE,A1,mu,Problem)
    %% Preparing Data
    ...
    elseif length(Last) > mu - sum(Next)
        ...
        while length(find(Next==1)) < mu
            ...
            PopNew   = [PopNew,SOLUTION(PopDec(Last(Rank(1)),:))];
            A2Obj    = [A2Obj; (PopNew.objs - zmin)./max(zmax - zmin,10e-10)];
