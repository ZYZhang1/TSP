classdef MaO_TSP < ALGORITHM
% <multi/many> <real> <expensive><constrained/none>
% Kriging-Assisted CMaOEA based on Two-Stage Probabilistic Penalty
    ...
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(A2)
                ...
                PopNew = NewSelect_many(PopDec,PopObj,PopCon,ObjMSE,ConMSE,A2,mu,Problem);
                ...
                if isempty(PopNew) == 0
                    ...
                    if sample_success
                        ...
                        elseif phase == 2
                            if Problem.FE >=  NI + (Problem.maxFE - NI)*3/4
                                if length(find(all(A2.cons<=0,2))) >= NI 
                                    phase = 3;
                                    [FrontNo,~]  = NDSort(A2.objs,A2.cons,1);
                                    fea_nd_obj   = A2(FrontNo==1).objs;
                                    Angle_Dis    = acos(1-pdist2(fea_nd_obj,fea_nd_obj,'cosine'));
                                    Angle_Dis(logical(eye(length(Angle_Dis)))) = inf;
                                    Angle_Dis    = sort(Angle_Dis,2);
                                    ND_Angle_new = max(Angle_Dis(:,1));
                                    
                                    if ND_Angle_new <= ND_Angle_old
                                        if Phase2_flag == 0
                                            Phase2_flag = 1;
                                        elseif Phase2_flag == 1
                                            phase = 3;
                                        end
                                    else
                                        Phase2_flag = 0;
                                    end
                                    ND_Angle_old = ND_Angle_new;
                                end
                            end
                        end
                ...
                % Population Update
                if sample_success
                    index = EnvironmentalSelection_many(A2.objs,A2.cons,NI);
                    A1    = A2(index);
                end
            end
        end
...
function [Model_obj,Model_con,THETA_obj,THETA_con] = model_train(A2,THETA_obj,THETA_con)
    ...
    for i = 1 : Len_obj
        ...
        dmodel     = dacefit(Dec(distinct,:),Obj(distinct,i),'regpoly0','corrgauss',THETA_obj(i,:),1e-5.*ones(1,Len_dec),100.*ones(1,Len_dec));
        Model_obj{i}   = dmodel;
...
function [PopDec,PopObj,PopCon,ObjMSE,ConMSE] = optimizaiton(A1,wmax,Model_obj,Model_con)
    ...
    while w <= wmax
        ...
        index  = SEnvironmentalSelection_many(PopDec,PopObj,ObjMSE,PopCon,ConMSE,length(A1));
